...
for my $mojo_db_config (@mojo_dbs_config) {
    for my $role (qw(Mojo::DB::Results::Role::MoreMethods +MoreMethods)) {
        ...
        throws_ok
            { $results->one_c_by_name('id', 'name', 'age', 'favorite_food') }
            qr/no rows returned/,
        ...
        my $c;
        lives_ok
            { $c = $results->one_c_by_name('id', 'name', 'age', 'favorite_food') }
            '1 row returned lives';
        isa_ok $c, 'Mojo::Collection';
        is_deeply $c, [1, 'Bob', 23, 'Pizza'];

        note 'Test passing only certain columns';
        $results = $db->select(people => '*' => {id => 1})->with_roles($role);
        is $results->rows, 1, '1 row returned';
